...
if [ "$#" -ne 3 ]; then
  echo "Usage: ./fullvideo_satisfyingclip.sh <YouTube URL> <Number of Clips> <Output File>"
  exit 1
fi
# Directory setup
mkdir -p ./clips
...
# Add blur top bottom borders to improve resolution retention
ffmpeg -i ./tmp/movie.mp4 -vf "gblur=sigma=20" -c:a copy ./tmp/blurred.mp4
...
for i in $(seq 1 "$2"); do
  # Generate a random clip length between 7-15 seconds
  length=$(($RANDOM % 9 + 7))
  ...
  ffmpeg -i ./tmp/edited.mp4 -ss $start_time -t 00:00:$length -c:v libx264 -c:a aac -preset fast -crf 22 ./clips/clip_${i}.mp4
done
